...
export const createCommentsService = async ({ postId, userId, content, parentId }) => {
    try {
        ...
        console.error("Create Comments Service Error:", error);
        throw new AppError(error.message || "Failed to create comment", 500);
    }
}
export const getCommentsService = async (postId) => {
    try {
        ...
        throw new AppError(error.message || "Failed to get comments", 500);
    }
}
export const deleteCommentsService = async (id, userId) => {
    try {
        ...
        console.error("Delete Comments Service Error:", error);
